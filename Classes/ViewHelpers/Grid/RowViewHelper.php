...
class RowViewHelper extends \TYPO3\CMS\Fluid\Core\ViewHelper\AbstractViewHelper {
	...
	public function render(\TYPO3\CMS\Media\Domain\Model\Media $media) {
		...
		$fields = \TYPO3\CMS\Media\Utility\TcaGrid::getInstance()->getFields();
		...
		foreach($fields as $fieldName => $configuration) {
			...
			if (\TYPO3\CMS\Media\Utility\TcaGrid::getInstance()->isNotInternal($fieldName)) {

				// Fetch value
				$value = call_user_func(array($media, 'getProperty'), $fieldName);

				if (\TYPO3\CMS\Media\Utility\TcaGrid::getInstance()->hasRenderer($fieldName)) {
					$renderer = \TYPO3\CMS\Media\Utility\TcaGrid::getInstance()->getRenderer($fieldName);
