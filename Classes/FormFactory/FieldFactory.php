...
class FieldFactory implements \TYPO3\CMS\Media\FormFactory\FieldFactoryInterface {
	...
	public function get(){
		...
		$className = '';
		if (\TYPO3\CMS\Media\Utility\TcaField::getInstance()->getFieldType($this->fieldName) == 'input') {
			$className = 'TYPO3\CMS\Media\FormFactory\TextFieldFactory';
		} elseif (\TYPO3\CMS\Media\Utility\TcaField::getInstance()->getFieldType($this->fieldName) == 'select') {
			#$className = 'TYPO3\CMS\Media\FormFactory\SelectFactory';
		} elseif (\TYPO3\CMS\Media\Utility\TcaField::getInstance()->getFieldType($this->fieldName) == 'check') {
			$className = 'TYPO3\CMS\Media\FormFactory\CheckboxFactory';
//		} elseif (\TYPO3\CMS\Media\Utility\TcaField::getInstance()->getFieldType($this->fieldName) == 'radio') {
//			// @todo implement me
		} elseif (\TYPO3\CMS\Media\Utility\TcaField::getInstance()->getFieldType($this->fieldName) == 'text') {
			$className = 'TYPO3\CMS\Media\FormFactory\TextAreaFactory';
		} elseif (\TYPO3\CMS\Media\Utility\TcaField::getInstance()->getFieldType($this->fieldName) == 'inline') {
			// @todo log not supported
		} elseif (\TYPO3\CMS\Media\Utility\TcaField::getInstance()->getFieldType($this->fieldName) == 'palette') {
			// @todo log not supported
		} else {
