...
class MediaFolder {
	 ...
	 * @return	array
	 */
	static public function findMediaFolders() {
	...
	/**
	 * Get a PID list for all media folders
	 *
	 * @return	string
	 */
	static public function getMediaFolderPidList() {
		$mediaFolders = self::findMediaFolders();
		if (empty($mediaFolders)) {
			self::createMediaFolder();
			$mediaFolders = self::findMediaFolders();
		}
		return implode(',', array_keys($mediaFolders));
	}
	 ...
	 * @return	mixed
	 */
	static public function createMediaFolder($pid=0) {
		/** @var $databaseHandle \TYPO3\CMS\Core\Database\DatabaseConnection */
		$databaseHandle = $GLOBALS['TYPO3_DB'];
