...
class ImageThumbnail extends \TYPO3\CMS\Media\Service\Thumbnail {
	...
	public function create() {
		...
		// so when naw_securedl don't add the timestamp
		$nawSecuredlLoaded = \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded('naw_securedl');
		...
		$thumbnail = sprintf('<img src="%s%s" title="%s" alt="%s" %s/>',
			...
			$nawSecuredlLoaded ? '' : '?'.($processedFile->isUpdated() ? time() : $processedFile->getProperty('tstamp')),
